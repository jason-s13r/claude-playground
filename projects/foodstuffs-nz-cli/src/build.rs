...
const COMMIT: &str = env!("FSNZ_COMMIT");
...
const BUILDER: &str = env!("FSNZ_BUILDER");
const RUSTC: &str = env!("FSNZ_RUSTC");
const TARGET: &str = env!("FSNZ_TARGET");
pub const PROFILE: &str = env!("FSNZ_PROFILE");
...
/// The running binary, with symlinks resolved: `fsnz update` replaces the real
...
/// The record `fsnz update` leaves behind, so a later `fsnz --version` can say
...
pub struct Install {
    ...
    /// different copy of fsnz and says nothing about this one.
    pub path: PathBuf,
