//! `twlnz` -- The Warehouse New Zealand from the command line.
//!
//! The interesting code is in `packages/`: the storefront protocol in
//! `twlnz-api`, the rendering in `cli-kit`. What is left here is the part that
...
fn main() -> ExitCode {
        ...
        .build()
    {
        Ok(rt) => rt,
        Err(e) => {
            eprintln!("twlnz: could not start the async runtime: {e}");
    ...
    match runtime.block_on(run(cli)) {
        ...
        Err(e) => {
            ...
            // thing would fix it. Only this binary knows it is called `twlnz`,
            ...
            // 2 misuse, 3 auth, 5 no store -- so a script can tell them apart
            // without reading this text.
