//! End-to-end: `wwnz update` against a stand-in GitHub.
//!
...
const CURRENT: &str = env!("WWNZ_VERSION");
...
async fn check_finds_the_newest_release_of_this_project_alone() {
    ...
    gh.releases(json!([
        ...
        gh.release("woolworths-nz-cli/v0.0.9", &[]),
        gh.release(&format!("woolworths-nz-cli/v{NEWER}"), &assets),
        // Flagged prerelease, and a semver prerelease too.
        gh.release("woolworths-nz-cli/v9.9.1-rc.1", &assets),
    ]))
    .await;
    ...
    // A pending update is a non-zero exit, so `wwnz update --check` can gate a
    ...
    assert_eq!(json["tag"], format!("woolworths-nz-cli/v{NEWER}"));
...
async fn check_is_happy_when_the_newest_release_is_the_one_running() {
    let f = Fixture::start().await;
    let gh = Github::start().await;
    // Nothing below CURRENT is mounted on purpose. dispat stamps the build
    // with the version it would release, which for a project with no tag yet
    // is 0.0.0 -- so no fixed version can be relied on to sort below it. The
    // other project's much higher tag is what proves the namespace filter is
    // doing the work here.
    gh.releases(json!([
        gh.release(&format!("woolworths-nz-cli/v{CURRENT}"), &[]),
        gh.release("other-project/v42.0.0", &[]),
    ]))
    .await;
...
async fn update_replaces_the_running_binary_and_records_where_it_came_from() {
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains(&format!("updating wwnz {CURRENT} -> {NEWER}")),
        "got: {stdout}"
    );
    assert!(stdout.contains("checksum verified"), "got: {stdout}");
    assert!(
        stdout.contains(&format!("installed wwnz {NEWER}")),
        "got: {stdout}"
    );
    ...
    assert_eq!(marker["tag"], format!("woolworths-nz-cli/v{NEWER}"));
...
async fn a_download_that_does_not_match_its_checksum_is_not_installed() {
    ...
    let gh = Github::start().await;
    let names = host_asset_names(NEWER);
    let archive = tarball("wwnz", b"#!/bin/sh\necho replaced\n");
...
async fn a_release_with_nothing_built_for_this_host_says_what_it_does_have() {
    ...
    gh.releases(json!([gh.release(
        &format!("woolworths-nz-cli/v{NEWER}"),
        &[
            format!("woolworths-nz-cli-{NEWER}-aix-ppc64.tar.gz"),
...
async fn the_version_says_when_update_is_what_put_the_binary_there() {
    ...
    std::fs::write(
        ...
        json!({
            "version": NEWER,
            "tag": format!("woolworths-nz-cli/v{NEWER}"),
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(stdout.contains(&format!("wwnz {CURRENT}")), "got: {stdout}");
    assert!(stdout.contains("binary"), "got: {stdout}");
    assert!(
        stdout.contains(&format!(
            "by `wwnz update` from woolworths-nz-cli/v{NEWER} on 2025-08-30"
        )),
        "got: {stdout}"
    );
}
