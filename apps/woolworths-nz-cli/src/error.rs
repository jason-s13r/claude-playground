...
//!
//! [`gsnz_core::Error`] is the interesting half -- it is what the adapter
//! produces and what carries the exit codes a script cares about. This enum
//! exists only because the app also has failures the retailer is not
//! responsible for: an unreadable config, a bad flag combination, a failed
//! self-update.
...
pub enum AppError {
    ...
    /// "wwnz: something is wrong" underneath it says less than the report
...
impl AppError {
    ...
    /// so a wrapper can tell "log in again" from "this site cannot do that".
