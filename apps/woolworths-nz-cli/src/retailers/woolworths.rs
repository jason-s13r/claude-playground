...
impl Woolworths {
        ...
        match &self.store_override {
            ...
            Some(_) => Err(Error::unsupported_hint(
                ...
                "the Woolworths cart is bound to a store server-side; \
                 run `wwnz store set <store>` instead",
            )),
        }
    }
    ...
    async fn category_key(&self, name: &str) -> Result<String> {
        ...
        root.find(name).map(|c| c.key.clone()).ok_or_else(|| {
            Error::Other(format!(
                "no Woolworths department matching {name:?}: run `wwnz departments`"
            ))
        })
    }
}
...
impl Retailer for Woolworths {
    ...
    async fn select_store(&self, id: &str) -> Result<Store> {
        let store = super::resolve_store(self.stores(None, u32::MAX).await?, id)?;
    ...
    async fn departments(&self) -> Result<Vec<Department>> {
        ...
        // which is a shape of the query rather than a level anyone browses;
        // returning its children is what makes `departments` a list of them.
        ...
        let filter = match filter {
            ...
            OrderFilter::InStore => {
                return Err(Error::unsupported_hint(
                    ID,
                    "in-store receipts",
                    "Woolworths keeps no till-receipt history, only its online orders",
                ))
            }
        };
        let held = self.account().await?;
    ...
    async fn login(
        ...
        let trace = move |step: &str, detail: &str| {
            if debug {
                eprintln!("wwnz: auth {step}: {detail}");
            }
        };
