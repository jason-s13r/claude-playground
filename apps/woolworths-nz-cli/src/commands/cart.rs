...
pub async fn run(app: &App, action: CartAction) -> AppResult<()> {
    ...
    let cart = match action {
        ...
        CartAction::Add {
            ...
            // Add is relative, so what is there now decides what to send: the
            // API takes an absolute quantity per line and has no increment.
        ...
        CartAction::Remove { sku } => {
            let current = handle.cart().await?;
            let key = resolve(&current, &sku);
            // Zero is how the site removes a line; there is no delete verb.
            handle
                .cart_apply(&[Change {
                    key,
...
///
/// Search prints a stock code and the API accepts only a variant key, and
/// people type the stock code. A line already in the cart knows its own key, so
/// match on that first and fall back to what was typed.
