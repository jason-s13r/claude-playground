...
//! (`woolworths-nz-cli/vX.Y.Z`), and every project in it releases on its own
...
const PROJECT: &str = "woolworths-nz-cli";
...
pub async fn releases(http: &wreq::Client) -> Result<Vec<Release>> {
    ...
    // Only ever to GitHub itself. WWNZ_UPDATE_API can point anywhere, and a
...
/// The release `wwnz update` should move to, or `None` when there is none.
///
...
/// A release by exact version, for `wwnz update <version>`.
...
fn extract(tarball: &[u8], expected: Option<&str>) -> Result<Vec<u8>> {
    ...
    for entry in archive.entries().context("reading the archive")? {
        ...
        if Some(name.as_str()) == expected || name == "wwnz" {
            return Ok(bytes);
        }
...
impl Staged {
    fn create(dir: &Path, exe: &Path) -> Result<Staged> {
        let name = exe.file_name().and_then(|n| n.to_str()).unwrap_or("wwnz");
        let path = dir.join(format!(".{name}.new-{}", std::process::id()));
        std::fs::File::create(&path).map_err(|e| {
            anyhow::anyhow!(
                "cannot write to {}: {e}\nwwnz installs over itself, so it needs write \
...
mod tests {
    ...
    fn only_this_projects_tags_are_recognised() {
        assert_eq!(
            version_from_tag("woolworths-nz-cli/v1.2.3"),
        ...
        assert!(version_from_tag("woolworths-nz/v1.2.3").is_none());
        assert!(version_from_tag("woolworths-nz-cli/v1.2").is_none());
    }
    ...
    fn release_with(assets: &[&str]) -> Release {
        Release {
            ...
            prerelease: false,
            tag: "woolworths-nz-cli/v1.0.0".into(),
            url: String::new(),
            assets: assets
                .iter()
                .map(|name| Asset {
    ...
    fn an_asset_is_matched_by_the_platform_in_its_name() {
        let names: Vec<String> = host_platforms()
            .iter()
            .map(|p| format!("woolworths-nz-cli-1.0.0-{p}.tar.gz"))
            .collect();
        ...
        let foreign = release_with(&["woolworths-nz-cli-1.0.0-aix-ppc64.tar.gz", "SHA256SUMS"]);
