...
//!
//! `wwnz -V` has to answer "which build of this is running?" for a binary that
//! could have arrived three ways: `cargo build` in a working tree, `cargo
//! install` from a checkout, or `wwnz update` pulling a release tarball. The
...
fn main() {
    ...
    emit("WWNZ_VERSION", Some(version.as_str()));
    emit("WWNZ_COMMIT", commit.as_deref());
    emit("WWNZ_COMMIT_DATE", commit_date.as_deref());
    emit("WWNZ_TAG", tag.as_deref());
    emit("WWNZ_REPO", repo.as_deref());
    emit("WWNZ_BUILDER", builder);
    emit("WWNZ_RUSTC", rustc.as_deref());
    emit("WWNZ_TARGET", std::env::var("TARGET").ok().as_deref());
    emit("WWNZ_PROFILE", std::env::var("PROFILE").ok().as_deref());
}
