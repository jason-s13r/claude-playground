...
/// `gsnz use ww` -- the one setting worth its own verb.
pub fn use_shop(app: &App, retailer: Option<RetailerId>) -> AppResult<()> {
    match retailer {
        Some(id) => write(app, "retailer", Some(id.id())),
        None => get(app, "retailer"),
    }
}
...
/// One value on its own line, so `$(gsnz config get retailer)` is usable.
fn get(app: &App, key: &str) -> AppResult<()> {
    ...
    match app.config.get(key)? {
        ...
        None => eprintln!("gsnz: {key} is not set"),
    }
    Ok(())
}
...
fn write(app: &App, key: &str, value: Option<&str>) -> AppResult<()> {
    ...
    // Read back what was stored rather than echoing what was typed: `ww` is
    // saved as `woolworths`, and showing the stored form is what makes the
...
mod tests {
    ...
    fn an_unknown_key_says_how_to_find_the_real_ones() {
        let err = Config::default().get("retailers").unwrap_err();
        assert!(err.to_string().contains("gsnz config list"), "{err}");
    }

    #[test]
    fn a_value_is_stored_in_its_canonical_form() {
        // `ww` is saved as `woolworths`, so the file reads the same however it
        // was written.
        let mut config = Config::default();
        config.set("retailer", "ww").unwrap();
        assert_eq!(
            config.get("retailer").unwrap().as_deref(),
            Some("woolworths")
        );
        config.set("retailer", "PAK'nSAVE").unwrap();
        assert_eq!(config.get("retailer").unwrap().as_deref(), Some("paknsave"));
    }

    #[test]
    fn a_bad_value_is_refused_at_the_point_of_writing_it() {
        let mut config = Config::default();
        assert!(config.set("retailer", "tesco").is_err());
        assert!(config.set("output.color", "purple").is_err());
        assert!(config.set("auth.store_password", "maybe").is_err());
        // One shop is not a comparison.
        assert!(config.set("compare.retailers", "nw").is_err());
        assert!(config.set("compare.retailers", "nw,ww").is_ok());
    }

    #[test]
    fn woolworths_has_no_token_command_and_says_why() {
        let err = Config::default()
            .set("woolworths.token_command", "print-token")
            .unwrap_err();
