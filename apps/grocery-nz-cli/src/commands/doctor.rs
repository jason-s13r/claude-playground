...
//! where its files are, which shop is the default. Then one section per shop,
//! ending in a live call, because "configured" and "working" are different
//! claims and only the second is worth much.
...
pub async fn run(app: &App) -> AppResult<()> {
    ...
    let report = Doctor {
        ...
        version: format!("gsnz {}", crate::build::short_version()),
        config_file: format!(
            "{} ({})",
