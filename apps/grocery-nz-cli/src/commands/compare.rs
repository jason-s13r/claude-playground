...
//! products and `fsnz` should get it back when it is rebuilt. What is here is
//! the fan-out, the flags, and reporting the shops that could not answer.
...
pub async fn run(app: &App, query: String, flags: Listing, strict: bool) -> AppResult<()> {
    ...
    if span.len() < 2 {
        return Err(AppError::usage(
            "comparing needs at least two shops: drop `-b`, or name several as `-b nw,pns`",
        ));
    }
    ...
    // A shop that answered nothing still gets a column: an empty one says "not
    // stocked here", which a missing column does not.
    ...
    for (id, e) in &failures {
        eprintln!("gsnz: {id} could not be included: {e}");
    }
    Ok(())
}

/// Every shop at once. One failure does not sink the rest: a lapsed Woolworths
/// session must not hide the two Foodstuffs prices.
async fn fan_out(
    handles: &[Handle],
    search: &Search,
...
/// Concurrent without a `futures` dependency: three shops means three tasks,
/// and `tokio::join!` needs a fixed arity this does not have.
