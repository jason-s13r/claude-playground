//! `kmart` -- Kmart Australia and New Zealand from the command line.
//!
//! The interesting code is in `packages/`: the four-surface protocol in
//! `kmart-api`, the rendering in `cli-kit`. What is left here is the part that
...
fn main() -> ExitCode {
        ...
        .build()
    {
        Ok(rt) => rt,
        Err(e) => {
            eprintln!("kmart: could not start the async runtime: {e}");
    ...
    match runtime.block_on(run(cli)) {
        ...
        Err(e) => {
            ...
            // thing would fix it. Only this binary knows it is called `kmart`,
            ...
            // 2 misuse, 3 auth, 5 no store, 6 no such thing, 7 rate limited,
            // 8 bot check -- so a script can tell them apart without reading
            // this text.
