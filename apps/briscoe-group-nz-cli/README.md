...
either catalogue, read a product, ask whether a shop has your size on the shelf,
and — signed in — work the cart, the wishlist, your orders and your loyalty
balance.
...
`compare` here the way there is between New World and PAK'nSAVE. They do not
share a sign-in either: separate Gigya sites, separate credentials, and
`auth status` always shows both so that is visible rather than surprising.
...
Rebel Sport shoe answers per size, which is the question worth asking:
...
`bgnz cart collect` is the other question — one verdict for the whole basket,
because that is what the service answers.

### Two order histories

`orders list` is what was bought online, out of the storefront, with carrier
tracking. `orders receipts` is what was bought standing in a shop, out of SAP,
matched to the loyalty membership and carrying the store and receipt reference.
Neither is a page of the other; they share no identifiers.

## Signing in

Gigya refuses a sign-in without a reCAPTCHA token, and refuses it *before* it
checks the password — so there is no way to do this with an HTTP client alone.
`auth login` drives a browser to mint that token:
...
The browser's whole job is the token. **The email and password never reach it** —
the sign-in request is made by this program once the token is in hand, which
also lets it ask for a session that outlives a browser tab. It needs
...
**Once per fascia.** What gets stored is Gigya's session token; everything after
that — including a fresh storefront token every two hours, forever — renews with
no browser and no password.

### Or skip the browser

The credential is a plain cookie on the site, so it can be copied out of
devtools without touching the network tab:
...
limited — so a script can tell them apart without reading the message.

## It looks like the others

`doctor`, `auth status` and the running totals on a cart or an order are the
same shape here as in `fsnz`, `kmart` and the rest — the label column, the
section headings and the `healthy` verdict all come from
[`cli-kit`](../../packages/cli-kit) rather than from a copy in this app.

## Where the code is

The interesting half is [`bgnz-api`](../../packages/bgnz-api), which speaks the
storefront protocol; rendering is [`cli-kit`](../../packages/cli-kit) and the
process boundary is [`net-kit`](../../packages/net-kit). What is left here is
the part genuinely about this program: reading the environment once, resolving
flags against config, driving the browser, and turning a failure into an exit
code.
