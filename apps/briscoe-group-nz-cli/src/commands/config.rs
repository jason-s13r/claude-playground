...
pub fn run(app: &App, action: ConfigAction) -> AppResult<()> {
    match action {
        ...
        ConfigAction::Path => {
            ...
            // Named per fascia because that is how they are stored, and
            // because "where did my Rebel Sport sign-in go" is a real question.
            for banner in bgnz_api::Banner::ALL {
                println!(
                    "  {:<11}{}",
                    banner.id(),
                    app.paths.scoped(banner.id()).state_dir.display()
                );
            }
        }
    }
