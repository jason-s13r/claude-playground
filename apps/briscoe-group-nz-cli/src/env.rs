...
pub struct Overrides {
    ...
    /// credential: URLs and operation names only, no headers and no bodies.
    pub debug: bool,
    pub no_color: bool,
    /// The login shell's path, which is how `completions` guesses which script
    /// to write when none is named.
    pub shell: Option<String>,
    /// The storefront, per fascia. Two variables rather than one because the
    /// two fascias are genuinely different hosts, and an integration suite
    /// pointing one at a mock server should not silently move the other.
    pub briscoes_origin: Option<String>,
    pub rebel_origin: Option<String>,
    /// SAP Customer Data Cloud, which both fascias share.
    pub gigya_origin: Option<String>,
    /// The browser the client presents as, by `wreq-util` profile name.
    ///
    /// Neither site is behind a bot manager today, so this is insurance rather
    /// than a load-bearing setting -- unlike its Warehouse equivalent.
    pub emulation: Option<String>,
    /// The interpreter that can `import camoufox`, when the one named by the
...
impl Overrides {
    ...
    pub fn read() -> Overrides {
        Overrides {
            config_dir: path("BGNZ_CONFIG_DIR"),
            state_dir: path("BGNZ_STATE_DIR"),
            secret_backend: var("BGNZ_SECRET_BACKEND"),
            update_api: var("BGNZ_UPDATE_API"),
            ...
            debug: flag("BGNZ_DEBUG"),
            // Set at all, to anything, means no colour. That is what the
            // convention says, so an empty value is not an override.
            no_color: std::env::var_os("NO_COLOR").is_some(),
            shell: var("SHELL"),
            briscoes_origin: var("BGNZ_BRISCOES_ORIGIN"),
            rebel_origin: var("BGNZ_REBEL_ORIGIN"),
            gigya_origin: var("BGNZ_GIGYA_ORIGIN"),
            emulation: var("BGNZ_EMULATION"),
            browser_python: var("BGNZ_BROWSER_PYTHON"),
        }
    }
}

/// An empty variable is treated as unset: `BGNZ_STATE_DIR=` in a shell script
...
/// Set to anything but a denial means on: `BGNZ_DEBUG=1` and `BGNZ_DEBUG=yes`
