...
/// Sign in once for the account, not once per banner.
///
/// `fsnz auth login` is the whole of setting this tool up: one prompt, because
/// one Club Plus account covers both New World and PAK'nSAVE. Asking for the
/// password twice -- once as each banner -- is the thing this avoids.
async fn login(
    app: &App,
    email: Option<String>,
    ...
    for target in targets {
        ...
        if app.config.auth.store_password && !no_store_password {
            ...
            if let Err(e) = net_kit::password::save(&secrets, &password) {
                eprintln!("fsnz: signed in, but the password could not be kept: {e}");
            }
        }
    ...
    for target in app.auth_targets() {
        ...
        match handle.auth_status().await {
            Ok(status) if !status.signed_in => {
                eprintln!(
                    "fsnz: {} is not signed in, so there is nothing to renew",
...
async fn status(app: &App) -> AppResult<()> {
    let shops = if app.selected.is_empty() {
        crate::retailers::BANNERS.to_vec()
    } else {
        app.selected.clone()
    };
...
/// `--json` gets the array, so `fsnz auth status --json | jq '.[0].signed_in'`
