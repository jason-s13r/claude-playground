...
pub async fn list(app: &App, query: Option<String>, limit: u32) -> AppResult<()> {
    ...
    emit(
        &mut app.out(),
        &StoreList::new(&stores).next("Select one: fsnz store set <id or name fragment>"),
    )?;
    Ok(())
}
...
async fn set(app: &App, needle: &str) -> AppResult<()> {
    ...
    if app_config.retailer.is_none() {
        // Choosing a store for a banner is as good a statement of "this is my
        // banner" as any, and saves a second command on first run.
    ...
    // fsnz store set ...", which after a successful `store set` reads as
