//! `fsnz completions` -- emit a shell completion script.
...
///
/// Nothing but the script goes to stdout, so `source <(fsnz completions zsh)`
    ...
    let shell = match shell.or_else(Shell::from_env) {
        ...
        None => bail!(
            "could not tell which shell this is from $SHELL; name one:\n  \
             fsnz completions <bash|zsh|fish|powershell|elvish>"
        ),
    };
    let mut cmd = Cli::command();
    clap_complete::generate(shell, &mut cmd, "fsnz", &mut stdout());
    Ok(())
}
