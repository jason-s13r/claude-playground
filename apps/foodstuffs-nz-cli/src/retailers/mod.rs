...
//!
//! `packages/` holds `fsnz-api`, which knows nothing about the domain, and
//! `gsnz-core`, which knows nothing about HTTP. Everything that reconciles them
//! lives here: the [`Retailer`] implementation, its conversions, and a
//! [`Registry`] that builds one on demand.
...
/// The two names one Foodstuffs platform trades under. `gsnz_core::RetailerId`
/// also carries Woolworths, which this tool never touches; anywhere the
/// combined tool would say `RetailerId::ALL`, this says `BANNERS`.
pub const BANNERS: [RetailerId; 2] = [RetailerId::NewWorld, RetailerId::PaknSave];

/// Whether an id is one this tool speaks for.
pub fn is_banner(id: RetailerId) -> bool {
    BANNERS.contains(&id)
}

/// Adapters, built once each and only when asked for.
///
/// `fsnz -b nw search` must not pay for a PAK'nSAVE client it will not use --
...
/// Filter a store list the way a person expects `fsnz stores wellington` to.
///
/// Foodstuffs returns every store and no city field, so the same filter is
/// applied here rather than at the server.
...
pub fn resolve_store(stores: Vec<Store>, needle: &str, id: RetailerId) -> Result<Store> {
    ...
    match matches.as_slice() {
        ...
        [] => Err(Error::Other(format!(
            "no {id} store matching {needle:?}: run `fsnz -b {} stores` to list them",
            id.short()
        ))),
...
///
/// `--size` is filtered here rather than at the server -- Foodstuffs has no
...
mod tests {
    ...
    fn nothing_matching_says_how_to_find_out() {
        let err = resolve_store(Vec::new(), "nowhere", RetailerId::PaknSave).unwrap_err();
        assert!(err.to_string().contains("fsnz -b pns stores"), "{err}");
    }
