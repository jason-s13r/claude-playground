...
//!
//! One variable is missing on purpose. `FSNZ_BANNER` is a `clap(env = ..)`
...
/// The escape hatches for one banner. These exist so the integration suite can
/// point the binary at a mock server, and so a broken default host can be
/// worked around without a release. The real hostnames live in `fsnz-api`.
#[derive(Clone, Debug, Default)]
pub struct Retailer {
    pub origin: Option<String>,
    /// The API host, where it differs from the storefront.
    pub api: Option<String>,
    pub store_id: Option<String>,
    /// A bearer token supplied outright, skipping acquisition. Per banner
    /// rather than global: a token is scoped to one banner, and one presented
    /// to the other is not refused -- it answers with that account's empty
    /// cart.
...
impl Overrides {
    ...
    pub fn read() -> Overrides {
        Overrides {
            config_dir: path("FSNZ_CONFIG_DIR"),
            state_dir: path("FSNZ_STATE_DIR"),
            secret_backend: var("FSNZ_SECRET_BACKEND"),
            update_api: var("FSNZ_UPDATE_API"),
            ...
            shell: var("SHELL"),
            newworld: Retailer::read("FSNZ_NEWWORLD"),
            paknsave: Retailer::read("FSNZ_PAKNSAVE"),
            clubplus: ClubPlus {
                origin: var("FSNZ_CLUBPLUS_ORIGIN"),
                api: var("FSNZ_CLUBPLUS_API"),
            },
        }
    }
}
...
/// An empty variable is treated as unset: `FSNZ_STATE_DIR=` in a shell script
