...
async fn an_asset_behind_a_redirect_is_downloaded_rather_than_reported_as_a_302() {
    ...
    // reqwest follows it because its builder defaults to ten redirects; the
    // sibling woolworths-nz-cli is on wreq, whose default is to follow none,
    // and that broke `update` outright. This pins the behaviour rather than
    // trusting a library default to stay put.
    let f = Fixture::start().await;
    let gh = Github::start().await;
    let payload = b"#!/bin/sh\necho redirected\n";
