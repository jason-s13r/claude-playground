...
so `Product` and `Store` here are the final types rather than a stop on the way
to one.

## Two countries, one backend

`kmart.com.au` and `kmart.co.nz` are the same service. The same GraphQL schema
answers on both hosts, store ids share a namespace across the pair — asking the
Australian gateway about location `8229` describes a shop in Auckland — and the
...
The Constructor index key and the Auth0 client id are Kmart's to rotate, and a
crate that carries them as constants stops working the day they do.
...
page — the same one a browser reads them from, and one of the few Kmart URLs a
plain HTTP client is served. The caller decides which to use; the CLI prefers
what it read, caches it for a week, and falls back to what shipped.

None of them is a secret — every visitor's browser downloads all of them — but
they are high-entropy strings that look exactly like leaked credentials, so
that one file is named in `.github/secret_scanning.yml`. The price of the
exemption is that nothing which *is* secret may go in it.
...
so there is no reason to scrape a page for something the index will hand over.
This crate parses no HTML except the Auth0 login form.

## What needs what

This is the fact that shapes the whole crate:
...
feed on a key the storefront ships in its own JavaScript. No credentials, no
browser, nothing to expire.
...
preflight a browser sends first. Nothing in a TLS fingerprint gets past it.

So gateway calls are admitted by cookies exported from a browser that already
passed the check, via [`auth::from_netscape`](src/auth.rs) — the same escape
hatch `wwnz-api` documents, and the reason `_abck` is treated as a credential.
...
Akamai guards the one step that would produce a session, and nothing else in
the flow. Its refusal is a `403` with an HTML body, which is indistinguishable
from a wrong password unless the body is read — so it is read, and
`Error::Challenged` comes back rather than `LoginRefused`. Telling someone to
check a password that was never seen is the worst answer available.

`auth::login` is kept regardless: it is correct against Auth0, it is one policy
change away from working, and it is the executable description of the flow.
...
session renews itself indefinitely — which makes the token half a far better
bargain than the cookies beside it, which expire daily.

`auth::login` walks Auth0's hosted login the way a browser walks it, with two
wrinkles the Woolworths flow does not have. Kmart's storefront is a single-page app, so it
generates its own PKCE pair rather than having a server do it — meaning this
crate does, and the RFC 7636 worked example is a unit test. And the flow ends
at a `code` on a redirect *to the storefront*, which is bot-checked: the
redirect is read, never followed, or the code is traded for a challenge page.
...
then the whole login, and most runs stop at the first. The last of those three
is the step Akamai blocks, so in practice a session that has lost its refresh
token is over.
...
and the channels are independent. An item can be sold out online and on a
shelf a suburb away, which is why `Availability` is not a boolean.

The four channels are shaped differently on the wire, and not consistently:
the two that ship have one pooled `stock`, the two that involve walking into a
building have `locations`. Asking for `stock` on `IN_STORE` is a schema error,
not an empty answer.
...
do not exist here, and that is deliberate rather than unfinished: the site's
own removal was never captured in a HAR, and introspection is disabled on the
gateway —
...
— so the operation name cannot be discovered. Guessing one would compile and
then fail at runtime, which is worse than not offering it. One capture of a
removal in a browser is all it needs.

## Loose parsing

Both JSON halves are undocumented and one is a search index whose fields differ
per category, so nothing is required and unfamiliar fields survive in
`Product::extra` rather than being dropped.

Two shapes worth knowing about, because both were found the hard way:
...
semantic match, so gibberish still answers with twenty products — which is why
`Listing::exact` is carried separately from `total`, and `Listing::is_guess`
exists to say so.
